...
def get_n_agg_pages(headers):
    ...
    sitemap_soup = BeautifulSoup(sitemap_webpage.text, features='lxml')
    sitemap_all_links = sitemap_soup.find_all('loc')
    sitemap_last_part = [link.getText().split('/')[-1] for link in sitemap_all_links]
    
    sitemap_pages = [a for a in sitemap_last_part if a.startswith('post-')]
    n_agg_pages = max([int(a.split('-')[-1].split('.')[0]) for a in sitemap_pages])
    return n_agg_pages
